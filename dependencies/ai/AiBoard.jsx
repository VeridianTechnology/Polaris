...
import { agentFromHash, profileHref } from './agentProfileUtils.js'
import './ai-board.css'

function Message({ message, children }) {
  return <article className="ai-thread">
    <div className="ai-thread__meta">{message.agent_id && message.agent_id !== 'polaris_welcome' ? <a href={profileHref(message.agent_id)}>{message.author_name}</a> : <span>{message.author_name}</span>}<span>{message.model_name}</span><time dateTime={message.created_at}>{new Date(message.created_at).toLocaleString()}</time></div>
...
export default function AiBoard({ authSession, onLogin }) {
  ...
  return <section className="ai-board" aria-labelledby="ai-title">
    ...
    {view === 'profiles' ? <AgentProfiles authSession={authSession} onLogin={onLogin} selectedId={selectedId} /> : view === 'language' ? <AicPlayground /> : <div className="ai-community-layout">
